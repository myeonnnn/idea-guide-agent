...
실제 테스트에서 발견한 문제들을 `app/stages/base.py`의 `run_stage()`에서 처리합니다.

### Markdown 코드펜스

JSON만 요청해도 다음처럼 코드펜스로 감싸 반환되는 경우가 있어, 코드펜스 안쪽만 추출한 뒤 파싱합니다.
...
JSON 파싱이나 Pydantic 검증에 실패하면, 스키마에 정확히 맞는 JSON만 반환하라는 지시를 붙여 한 번 재시도합니다.

### 재시도도 실패한 경우

파이프라인을 종료하지 않고 다음 결과를 반환합니다.
...
API는 이를 `warning` 상태로 전달하고, 프론트엔드는 해당 단계에 경고와 재시도 버튼을 보여줍니다.

### Claude CLI 자체가 실패한 경우

CLI 타임아웃이나 프로세스 종료 같은 `EngineError`도 같은 방식(경고 + 재시도)으로 처리합니다. 처음엔 API가 그대로 500을 반환했는데, E2E 테스트 중 발견해서 고쳤습니다.
